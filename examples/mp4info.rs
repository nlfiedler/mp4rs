...
fn get_framerate(sample_counts: &Vec<u32>, duration: u64, timescale: u32) -> String {
    let sc = (sample_counts[0] as f64) * 1000.0;
    let ms = (duration as f64 / timescale as f64) * 1000.0;
    return format!("{:.2}", sc / ms.floor());
}

fn format_creation_time(creation_time: u64) -> String {
    let time = if creation_time >= 2082844800 {
        // seconds between 1904-01-01 and epoch
